	...
	return (
		<DashboardPage>
			...
			<div className="dashbord-mini-info-layout">
				...
				<InfoWidget label="Stage Archivés" icon={<TbSchool/>} info={0} color="var(--blue-col)"/>
			</div>


			{archivedTerList.length > 0 ? (
				<div>
					{archivedTerList.map(ter => (
						<TERWidget key={ter.id} period={ter} onClick={() => clickHandle(ter.id)}/>
					))}
				</div>
			) : (
				<EmptyWidget icon={<FiArchive size={30}/>} text="Aucune période archivée pour le moment."/>
			)}
		</DashboardPage>
	)
}
